...
class FoodLensHomeScreen extends ConsumerStatefulWidget {
  const FoodLensHomeScreen({super.key});

  @override
  ConsumerState<FoodLensHomeScreen> createState() => _FoodLensHomeScreenState();
}

class _FoodLensHomeScreenState extends ConsumerState<FoodLensHomeScreen> {
  ...
  void _triggerImageScan({bool fromCamera = false}) {
    ImagePickerHelper.pickImage(
      (base64, name, filePath) async {
        ...
        final notifier = ref.read(foodVisionProvider.notifier);
  ...
  void _startBarcodeLookup(String barcode) async {
    _showLoadingOverlay();
    await ref.read(foodVisionProvider.notifier).scanAndAnalyze(barcode: barcode);
  ...
  void _startRegistryProductLookup(Map<String, dynamic> rawDetails) async {
    ...
    await ref.read(foodVisionProvider.notifier).scanAndAnalyze(barcode: code, searchName: name, rawDetails: rawDetails);
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text(
                    'Analyzing ingredients & scoring food...',
  ...
  void _dismissLoadingAndNavigate(String? barcode) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => FoodProductDetailScreen(barcode: barcode),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            const Icon(Icons.breakfast_dining_rounded, color: AppTheme.accentCyan, size: 24),
          ...
          Expanded(
            child: AnimatedSwitcher(
              ...
              child: _searchResults.isNotEmpty
                  ...
                  : _buildHistoryList(foodState.history),
            ),
          ),
        ],
      ),
  ...
  Widget _buildSearchResultsList() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, idx) {
        ...
        final name = item['product_name']?.toString() ?? 'Unknown Food';
  ...
  Widget _buildHistoryList(List<FoodProduct> history) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    if (history.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            ...
            const Text('Scan food packages to view details.', style: TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
          ],
        ),
      );
    }
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              GestureDetector(
                onTap: () => ref.read(foodVisionProvider.notifier).clearHistory(),
